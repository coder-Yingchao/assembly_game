...
import  random
import pandas as pd
# Modify the assembly process from two steps (pick up and then place a part) to one step.
class ComputerAssemblyGame:
    def __init__(self):
        ...
        excel_file = './assets/muscle_fatigue_results.xlsx'  # Replace with the path to your Excel file
        self.df = pd.read_excel(excel_file)
        # Requesting executable tasks
        self.human_capability = [1, 1, 1, 1, 1, 1, 1, 1]
        self.robot_capability = [0, 0, 1, 1, 1, 1, 1, 0]
    ...
    def reset_game(self, seed):
        ...
        # self.write_log_header()
        # self.create_action_log_file()
    def move_with_random_action(self, action_hand1, action_hand2):
        executable_tasks = self.get_executable_tasks()
        do_human = random.randint(0,3)
        rand_task = 0
        ...
        if not do_human == 0:
            return self.move_hands_simultaneously(-1, action_hand2)
        else:
            if rand_task == -1:
                return self.move_hands_simultaneously(-1, action_hand2)
            else:
                ...
                if int(task_name[0]) == action_hand2:
                    action_hand2 = -1
                return self.move_hands_simultaneously(int(task_name[0]), action_hand2)
        ...
        if hand_id == 1:
            ...
            for num_comp in range(len(self.components)):
                ...
                if comp.picked_up_by_hand1:
                    ...
                    self.reward_agent2 = self.reward_agent2 - self.df.iloc[num_comp].sum() *10 - self.df.iloc[num_comp].max()*10
    ...
    def assemble_part(self, hand_id, num_comp):
        ...
        """
        if hand_id == 1:
            if self.human_capability[num_comp]==0:
                self.reward_agent1 += self.fail_action_reward
                return
        else:
            if self.robot_capability[num_comp]==0:
                self.reward_agent2 += self.fail_action_reward
                return
        comp = self.components[num_comp]
        executable_tasks = self.get_executable_tasks()
        if (comp.type in executable_tasks) and (self.states[comp.type]['state'] == 1):
            ...
            if hand_id == 1:
                ...
                self.reward_agent1 += self.assemble_success_reward
                self.reward_agent2 = self.reward_agent2 - self.df.iloc[num_comp].sum() * 10 - self.df.iloc[
                    num_comp].max() * 10
            else:
                print('hand2 assemble')
                self.reward_agent2 += self.assemble_success_reward
