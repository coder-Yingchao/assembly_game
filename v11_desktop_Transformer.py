...
if __name__ == "__main__":
    ...
    envs = DummyVectorEnv([lambda: make_env() for _ in range(8)])
    test_envs = DummyVectorEnv([lambda: make_env() for _ in range(4)])
    ...
    log_path = os.path.join(args.logdir, "assemblygame_desktop_v11", "DQN_Transform_1")
    ...
    def save_best_fn(policy):
        os.makedirs(os.path.join("log", "assemblygame_desktop_v11", "DQN_Transform_1"), exist_ok=True)

        model1_save_path = os.path.join("log", "assemblygame_desktop_v11", "DQN_Transform_1", "policy.pth")
