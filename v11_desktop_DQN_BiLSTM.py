...
def make_env():
    env = gym.make('ComputerAssemblyEnv-v11')
    return env
...
if __name__ == "__main__":
    ...
    envs = DummyVectorEnv([lambda: make_env() for _ in range(4)])
    ...
    log_path = os.path.join(args.logdir, "assemblygame_desktop_v11", "DQN_BiLSTM_50")
    ...
    def save_best_fn(policy):
        os.makedirs(os.path.join("log", "assemblygame_desktop_v11", "DQN_BiLSTM_50"), exist_ok=True)

        model1_save_path = os.path.join("log", "assemblygame_desktop_v11", "DQN_BiLSTM_50", "policy.pth")
