...
from tianshou.utils.net.common import Net, ActorCritic, Recurrent, Transformer, DQNResNet
...
if __name__ == "__main__":
    ...
    if True:
        policy.load_state_dict(torch.load("log/assemblygame_desktop_v9_2/Dueling_DQN_Relu/policy.pth"))
    ...
    while running:
        ...
        observation = init_obs['obs']
        act = policy.compute_action(observation, info, state)
        print('executable_task:', env.env.game.get_executable_tasks())

        print('robot action: ',  act)
        observation, reward, term, truncated, info = env.step(act)
        ...
        print('human action: ',current_action_agent1)
        observation, reward2, term2, truncated2, info2 = env.step(current_action_agent1)
