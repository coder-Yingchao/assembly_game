...
if __name__ == "__main__":
    ...
    envs = SubprocVectorEnv([lambda: make_env() for _ in range(10)])
    test_envs = SubprocVectorEnv([lambda: make_env() for _ in range(2)])
    # envs = SubprocVectorEnv([lambda: make_env() for _ in range(1)])
    ...
    log_path = os.path.join(args.logdir, "assemblygame_pump_random_v9", "DDQN_ResN")
    ...
    def save_best_fn(policy):
        os.makedirs(os.path.join("log", "assemblygame_pump_random_v9", "DDQN_ResN"), exist_ok=True)

        model1_save_path = os.path.join("log", "assemblygame_pump_random_v9", "DDQN_ResN", "policy.pth")
