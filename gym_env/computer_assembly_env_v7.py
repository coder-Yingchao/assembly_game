...
import pygame
import random
from game.computer_assembly_game_v5 import ComputerAssemblyGame
from gymnasium.envs.registration import register
# gym env, which has only one agent.
register(
    id='ComputerAssemblyEnv-v7',  # Use an environment ID with a version number
    entry_point='gym_env.computer_assembly_env_v7:ComputerAssemblyEnv',  # Module path : ClassName
)
...
class ComputerAssemblyEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.action_space =  spaces.Discrete(11)  # Each hand can perform 9 actions
    ...
    def step(self, action):
        ...
        rewards, terminated = self.game.move_with_random_action( -1, action-1)
        reward  = rewards[1]
