...
register(
    id='ComputerAssemblyEnv-pump-v11',  # Use an environment ID with a version number
    entry_point='gym_env.computer_assembly_env_pump_v11:ComputerAssemblyEnv',  # Module path : ClassName
)
...
class ComputerAssemblyEnv(gym.Env):
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "mask": spaces.Box(low=0, high=1, shape=(27,), dtype=np.int8),
            }
        )
    ...
    def step(self, action):
        ...
        # print('current step:', current_step, 'done:', done)

        # Reset the step counter if the episode is done
        if done:
            current_step = 0
            # print(f'{reward}')
        info = {'reward_agent1_part1_step': rewards_agent1_parts[0],
                'reward_agent1_part2_completion': rewards_agent1_parts[1],
                'reward_agent1_part3_failAction': rewards_agent1_parts[2],
    ...
    def _get_observation(self):
        ...
        executable_tasks = self.game.get_executable_tasks()

        mask = np.zeros(27, "int8")
        mask[0] = 1
        for task in executable_tasks:
            mask[int(task)] = 1

        # robot_capability = self.game.robot_capability
        # for i in range(len(robot_capability)):
        #     if robot_capability[i] == 0:
        #         mask[i + 1] = 0
